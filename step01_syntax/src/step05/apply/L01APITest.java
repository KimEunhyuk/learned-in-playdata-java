...
public class L01APITest {
	...
	public static void main(String[] args) {
		...
		// -3 값 주고 절대밗(abs) 반환 int 주고 int 반환 받아 출력
		System.out.println(Math.abs(-3));
		
		//? 저 문자열에서 기 하나만 출력
		// 힌트 - 문자열 하나한의 글자에 번호 부여, 0부터 시작
		// 기 - 0번째 글자
		/* API 개발자 관점
		 * 문자열(String 객체) 데이터는 존재하는 전제하게
		 * 숫자받아서(argument) 해당 숫자 위치값을 글자 하나(char)를 뽑음 -> 반환
		 * 
		 * char ?(int index){ 문자열 객체에서 index 내의 해당 char 착출및 반환}
		 * 
		 * 경우의 수 고려해 보기
		 * - 호출시 class명으로만 호출 가능 단, index로 데이터 뽑을 수 있는 데이터가 없음
		 * - 논리적으로 문제 발생
		 * - 이런식의 개발은 안 된 다 (욕!)
		 ...
		 * 		parameter로 받은 문자열 객체에서 index 내의 해당 char 착출 빛 반환
		 * }
		 * 
		 */
		String data = "기술이 지적자산 가치 뽐뿌 - 기원";
		System.out.println(data.charAt(0));		// 기
		 ...
		 */
		System.out.println(Integer.parseInt("two")); //안됨

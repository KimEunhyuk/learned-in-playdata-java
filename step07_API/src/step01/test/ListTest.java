 ...
 * 		- 기본값들을 객체처럼 표현가능하게 해주는 8가지 도우미 클래스들 존재
 * 		- int : Integer(클래스, 참조 타입) / byte : Byte (클래스, 찹조 타입)등
...
//		- 객체 타입인 경우 무조건 모두 다 저장
	public static void m() {
		...
		al.add("문자열"); //0번째로 저장
		al.add(new Integer(3)); //index1에 저장
		//al.add(3); 컴파일시 al.add(new Integer(3))으로 자동 변경 
		al.add("문자열2");
		al.add(new Integer(3));
		
		//저장된 데이터 개수 확인 - 메모리 개수 확인이 아님
		System.out.println(al.size());
		
		//모든 데이터 단순 출력
		System.out.println(al);
		//al.toString() 재정의 : 보유된 모든 데이터값 하나의 문자열로 반환
		
		//index 2번째의 문자열2삭제후 size()호출
		al.remove(2);
	 ...
	 * public ? get(?){ 해당 index에 저장되어 있는 데이터 반환}
	...
	public static void m2() {
		...
		al.remove(2); //해당 index 삭제 하면서 뒤에 저장되어있던 데이터들 index 재정렬
	...
	//두번째 타입을 제한할 수 있는 jdk1.5부터 제시된 제네렉(제네릭) 문법
	...
	/*
	 * 고객의 모든 정보를 관리자 화면에 출력을 위해 db로부터 select -> CustomerDTO 객체들 생성
	 *  -> ArrayList에 저장 -> 반환
	 */
	public static ArrayList<CustomerDTO> getCustomers(){
		ArrayList<CustomerDTO> all = new ArrayList<CustomerDTO>();//10개메모리생성

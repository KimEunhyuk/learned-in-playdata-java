...
public class IOTest {
	 ...
	 * 콘솔 - System.out
	 * 한글 데이터 입출력 하고싶은데?
	 * 	- 콘솔 출력시 콘솔지정 - System.out / 출력 println()
	 * 	buffer 기능 활용
	 * 		BufferedReader - enter 기준으로 문자열을 구분해서 한 문자열로 read 가능한 메소드 지원
	 * System.in - 1byte / BufferedReader - 2byte
	 * 	- 1byte와 2byte 호환 API필요
	 * 		- InputStreamReader 로 호환
	 * 
	 * BufferedRead(Reader in){}  -2  / 여기에는 System.in 못들어옴 그래서 밑에 들어왔다가 와야함
	 * InputStreamReader(InputStream in){}  -1 / 1->2 바꿔줌
	 * 1 -> 2 , System.in - 1
	 * 
	...
	public static void m2() throws IOException{
		...
		while(data != null){
			out.write(data);
			data = in.readLine();
			out.flush(); //buffer 메모리에 잔졸될수도 있는 데이터를 출력하겠금 해주는 기능
		}
...
//				out.newLine();	//new line 적용하는 메소드
	...
	//playdata.txt file read해서 encoir.txt에 출력
...
//				out.newLine();	//new line 적용하는 메소드
	...
	/*
	 * mu.properties 생성 및 데이터 저장
	 * 	- key = value
	 * 
	 * my.properties내에 있는 특정 key에 해당하는 value값만 출력
	 * 개발시에 고려사항
	 * my.properties 파일을 활욜 API Properties  클래스
	 */
...
//			stuInfo.load(new FileReader("my.properties"));
			...
			//steo07_API/src/my.properties 경로도 정상으로 개발해 보기

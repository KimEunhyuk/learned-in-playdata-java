...
//		insert();
	...
	/*
	 * 현업 개반자
	 * parameter 즉 insert 하는 데이터는 브랑줘에서 입력받는 가변적인 데이터
	 * 
	 * 설계 1 - 컬럼수 즉 insert 하고자 하는 데이터 수 만큼 parameter 선언
	 * 설계 2 - dto형식으로 parameter 선언(권장)
	 * 	단, pk 하나 값으로 하나의 컬럼값 수정시에는 두개의 parameter값 선언이 더 적합
	 * 
	 * 고려사항
	 * 	*** 정상 저장 또는 비정상 저장에 대한 모든 상황은 client는 알아야 함
	 * 		deptno가 중복되었다는 메세지는 당연히 client가 알아야 할 상황
	 * 		db접속 문제가 즉 서버 내부 문제 발생시에 신뢰를 유지하는 범위내에서 재시도 유도
	 * 
	 *  	결론 : DAO의 crud 메소드는 예외를 throws로 던짐
	 *  		controller에서 예외 발생시 fail view 화면으로 정상 응답 처리 
	 *  
	 *  
	 * 1. 정상 저장
	 * 	- 새로운 데이터 저장
	...
	public static void insert() throws Exception{
		//try 블록내에서 선언된 변수는 try 외부에선 사용 불가(로컬)
		Connection con = null;
		Statement stmt = null;
